...
module.exports = {
  listeners: {
    ...
    move: state => function (movementCommand) {
      ...
      if (door) {
        if (door.locked) {
          return B.sayAt(this, "The door is locked. Try using 'unlock north/south/east/west'");
        }

        if (door.closed) {
          return B.sayAt(this, "The door is closed. Try using 'open north/south/east/west'");
        }
      }
    ...
    commandQueued: state => function (commandIndex) {
      ...
      B.sayAt(this, `<b>Executing '${command.label}' in ${ttr} seconds.</b>`);
    },
    ...
    experience: state => function (amount) {
      ...
      if (this.experience + amount > totalTnl) {
        ...
        while (this.experience + amount > nextTnl) {
          ...
          B.sayAt(this, `<b>You are now level ${this.level}!</b>`);
          this.emit('level');
        }
      }
