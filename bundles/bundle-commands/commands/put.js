...
module.exports = {
  ...
  command : (state) => (args, player) => {
    ...
    if (colonsplit.length > 2){
      ...
      colonsplit = colonsplit.join(" ").split(" ").filter(arg => !arg.match(/in/) && !arg.match(/into/)).filter(arg => arg != '');
    } else {
      colonsplit = args.split(' ').filter(arg => !arg.match(/in/) && !arg.match(/into/));

    }

    // put 3.foo in bar -> put 3.foo bar -> put 3.foo into bar
//    const parts = args.split(' ').filter(arg => !arg.match(/in/) && !arg.match(/into/));
    const parts = colonsplit;
