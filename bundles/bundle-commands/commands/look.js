...
function lookRoom(state, player) {
  ...
  if (player.room.coordinates) {
    ...
    //B.sayAt(player, B.line(20) + B.line(5, ' ') + line2);
    //B.sayAt(player, B.line(20, ' ') + '<yellow><b>' + line3 + '</b></yellow>');
  }

  if (!player.getMeta('config.brief')) {
    B.sayAt(player, "" + room.description + "");
  }
  ...
  room.items.forEach(item => {
    if (item.hasBehavior('resource')) {
      B.sayAt(player, `[${ItemUtil.qualityColorize(item, 'Resource')}] <code>${item.roomDesc}</code>`);
    } else {
      B.sayAt(player, `[${ItemUtil.qualityColorize(item, 'Item')}] <code>${item.roomDesc}</code>`);
    }
  });
  ...
  room.npcs.forEach(npc => {
    ...
    B.sayAt(player, `[${npcLabel}] <code>` + npc.name + "</code>" + combatantsDisplay );
  });

  var exitsMsg = "";

//  B.at(player, '[<yellow><b>Exits</b></yellow>: ');
  exitsMsg +='';
  ...
  exitsMsg += foundExits.map(exit => {
    const exitRoom = state.RoomManager.getRoom(exit.roomId);
    const door = room.getDoor(exitRoom) || (exitRoom && exitRoom.getDoor(room));
    if (door && (door.locked || door.closed)) {
      return '' + exit.direction + '(locked)';
    }
...
//  B.sayAt(player, ']');
  ...
  B.sayAt(player, "[<b>Exits</b>: <code>"+exitsMsg+"</code>]<buttons>"+exitsMsg.replaceAll("(locked)","")+"<buttons>");
...
function lookEntity(state, player, args) {
  ...
  //ENABLE 'names' in arguments
  let colonsplit = args.split("'");
  console.log("colonsplit before: "+ colonsplit);
  if (colonsplit.length > 2){
    for (var i = 1;i<colonsplit.length;i+=2){ 
      colonsplit[i] = colonsplit[i].trim().replace(' ','-'); 
    }
    colonsplit = colonsplit.join(" ").split(" ").filter(arg => !arg.match(/from/)).filter(arg => arg != '');
  } else {
    colonsplit = args.split(' ').filter(arg => !arg.match(/from/));
  }
  ...
  room.emit("lookItem", entity, player, room);

  B.sayAt(player, ""+ entity.description + "");
