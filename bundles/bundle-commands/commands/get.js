  ...
  command : (state) => (args, player, arg0) => {
    ...
    // most recent corpse. See issue #247.
      ...
      if (container.type !== ItemType.CONTAINER) {
        return Broadcast.sayAt(player, `${ItemUtil.display(container)} isn't a container. If the name of the item has more than one word, try using single quotes. e.g. <b>get 'iron key'</b>`);
      }
    ...
    if (!item) {
      return Broadcast.sayAt(player, "You don't see anything like that here.\nIf the item you want is on the desk or inside a box, use <b>get 'item name' from 'desk' to get it</b>");
    }
