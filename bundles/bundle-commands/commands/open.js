...
module.exports = {
  ...
  command: state => (args, player, arg0) => {
    ...
    if (roomExit) {
      ...
      if (door) {
        return handleDoor(player, doorRoom, targetRoom, door, action, state);
      }
    }
...
function handleDoor(player, doorRoom, targetRoom, door, action, state)
{
  switch (action) {
    case 'open': {
      if (door.locked) {
    ...
    case 'unlock': {
      ...
      if (door.lockedBy) {
        const key = state.ItemFactory.create(door.area, door.lockedBy);
        if (player.hasItem(door.lockedBy)) {
          const playerKey = player.hasItem(door.lockedBy);
          B.sayAt(player, `*click* You unlock the door with ${ItemUtil.display(key)}.`);
          return doorRoom.unlockDoor(targetRoom);
        }

        return B.sayAt(player, `The door can only be unlocked with ${ItemUtil.display(key)}.`);
      }
